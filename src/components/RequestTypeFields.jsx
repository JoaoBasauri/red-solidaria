  ...
  return <section className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4">
    ...
    <div className="mt-3 grid gap-4 sm:grid-cols-2">
      {requestFields(type, values).map(([name, label, inputType, required, extra]) => {
        ...
        return <label key={name} className="block text-sm font-bold text-[#073164]">{label}{required ? ' *' : ''}<input name={name} type={inputType === 'url' ? 'text' : inputType} inputMode={inputType === 'url' ? 'url' : undefined} placeholder={inputType === 'url' ? 'dominio.com' : undefined} required={required} value={values[name] || ''} onChange={onChange} {...extra} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" /></label>
      })}
    </div>
