-- Seis solicitudes aprobadas para demo, una por tipo. Idempotente por codigo.
begin;
...
select id,datos->>'tipo_emergencia',(datos->>'fecha_emergencia')::date,(datos->>'poblacion_afectada')::integer,datos->>'necesidades_urgentes' from public.solicitudes where codigo='RS-DEMO-APROBADA-001' on conflict(solicitud_id) do nothing;
...
select id,datos->>'tipo_kit',(datos->>'cantidad_solicitada')::integer,(datos->>'poblacion_beneficiaria')::integer,(datos->>'fecha_necesidad')::date,datos->>'condiciones_especiales' from public.solicitudes where codigo='RS-DEMO-APROBADA-002' on conflict(solicitud_id) do nothing;
...
select id,datos->>'tipo_recurso',datos->>'descripcion_recurso',(datos->>'cantidad')::numeric,datos->>'unidad',(datos->>'disponibilidad_desde')::date,(datos->>'disponibilidad_hasta')::date from public.solicitudes where codigo='RS-DEMO-APROBADA-003' on conflict(solicitud_id) do nothing;
insert into public.solicitud_aliado(solicitud_id,tipo_aliado,nombre_organizacion,cobertura,capacidades,sitio_web)
select id,datos->>'tipo_aliado',datos->>'nombre_organizacion',datos->>'cobertura',datos->>'capacidades',datos->>'sitio_web' from public.solicitudes where codigo='RS-DEMO-APROBADA-004' on conflict(solicitud_id) do nothing;
insert into public.solicitud_voluntario(solicitud_id,disponibilidad,habilidades,zona_preferida,fecha_nacimiento)
select id,datos->>'disponibilidad',datos->>'habilidades',datos->>'zona_preferida',(datos->>'fecha_nacimiento')::date from public.solicitudes where codigo='RS-DEMO-APROBADA-005' on conflict(solicitud_id) do nothing;
...
select id,datos->>'nombre_propuesto',datos->>'responsable_nombre',datos->>'responsable_telefono',datos->>'horario_propuesto',(datos->>'fecha_inicio')::date,(datos->>'fecha_fin')::date,datos->>'capacidad_descripcion' from public.solicitudes where codigo='RS-DEMO-APROBADA-006' on conflict(solicitud_id) do nothing;
...
where destinatario in (select email_solicitante from public.solicitudes where codigo like 'RS-DEMO-APROBADA-%') and estado in ('PENDIENTE','ERROR');
